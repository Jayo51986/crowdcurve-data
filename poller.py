...
def fetch_spots():
    spots = {}
    # Primary: Coinbase spot. Not geo-blocked on GitHub runners, no key.
    for sym, c in COINS.items():
        if not c.get("cb"):
            continue
        d = get_json(f"https://api.coinbase.com/v2/prices/{c['cb']}/spot")
        try:
            if d and "data" in d and "amount" in d["data"]:
                spots[sym] = float(d["data"]["amount"])
        except (TypeError, ValueError, KeyError):
            pass
        time.sleep(0.1)
    # Fallback for anything missing (incl. BNB, HYPE): CoinGecko free endpoint.
...
def fetch_kalshi():
    ...
    base = None
    for h in hosts:  # find a host that answers
        probe = get_json(h + "?status=open&limit=1")
        if probe and "markets" in probe:
            base = h
            log(f"kalshi host ok: {h}")
            break
    if not base:
    ...
    seen = 0
    for _ in range(40):
        url = base + "?status=open&limit=1000" + (f"&cursor={cursor}" if cursor else "")
        d = get_json(url)
        ...
        for mkt in d["markets"]:
            ...
            try:
                ...
                if not sym:
                    continue
                text = str(mkt.get("title") or "") + " " + str(mkt.get("subtitle") or "") + " " + str(mkt.get("yes_sub_title") or "")
                strike = None
                direction = None
                if mkt.get("floor_strike") is not None:
                    strike = float(mkt["floor_strike"]); direction = "above"
                elif mkt.get("cap_strike") is not None:
                    strike = float(mkt["cap_strike"]); direction = "below"
                else:
                    strike = parse_price(text); direction = parse_direction(text)
                if strike is None or direction is None:
                    continue
                prob = None
                ...
                out.append({"venue": "k", "coin": sym, "strike": strike,
                            "prob": clamp(prob), "vol": float(mkt.get("volume") or 0), "end": end, "q": text.strip()})
            except (TypeError, ValueError, KeyError):
                continue
        ...
        if not cursor:
            break
        time.sleep(0.5)
    log(f"kalshi: scanned {seen} open markets, kept {len(out)} crypto threshold markets")
    return out
